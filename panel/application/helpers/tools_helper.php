...
// $model file_model veya  image_model gelirse buradaki işlemleri yapacak
    if ($model == ("image_model" || "file_model")){
        $galleries = $t->gallery_model->get(
            array(
                "id" => $fileName->gallery_id
            )
        );

        $galleriesType = $galleries->gallery_type == "image" ? "images" : "files";

        return unlink("uploads/{$t->viewFolder}/{$galleriesType}/{$galleries->url}/{$resolution}/{$fileName->url}");
    }

// Diğer tüm silinecekler için bu işlem geçerli
